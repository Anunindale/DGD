 ...
 * @author riaan
 */
public class ItemDimension2SetupForm extends BaseInternalFrame {
    ...
    public ItemDimension2SetupForm(EMCUserData userData) {
        super("Sizes", true, true, true, true, userData);
        copyUD = userData.copyUserDataAndDataList();
        this.setBounds(20, 20, 600, 290);
        this.setHelpFile(new emcHelpFile("Inventory/InventorySize.html"));
        try {

            dataRelation = new EMCControlLookupComponentDRM(new emcGenericDataSourceUpdate(
                    enumEMCModules.INVENTORY.getId(), new emc.entity.inventory.dimensions.datasource.InventoryItemDimension2SetupFormDS(), userData), userData) {

                @Override
                public void updatePersist(int rowIndex) {
                    if (isRowUpdated()) {
                        ItemDimension2SetupForm.this.changesMade = true;
                    }
                ...
                public void deleteRowCache(int rowIndex) {
                    if ((Long)getLastFieldValueAt("recordID") != 0) {
                        ...
                        toSend.add(EnumDimensions.DIMENSION2.toString());
            ...
            lkpItem = EMCItemLookupFactory.createItemControllLookup(dataRelation, txtItemDesc, InventoryItemDimension2Setup.class.getName(), copyUD);
    ...
    private void setupLookupRelation() {
        ...
        query.addTableAnd(InventoryItemMaster.class.getName(), "dimensionGroupId", InventoryDimension2GroupSetup.class.getName(), "dimension2Group");
        query.addAnd("itemId", emc.functions.Functions.checkBlank(lkpItem.getValue()) ? "" : lkpItem.getValue().toString(), InventoryItemMaster.class.getName());

        lkpDimension.setTheQuery(query);

        Dimension2LookupRelationManager lookupRelasion = new Dimension2LookupRelationManager();

        lookupRelasion.addLookup(lkpItem, "item");
        lookupRelasion.addLookup(lkpDimension, "dimension");

        //  lookupRelasion.initializeLookups();
    }
    ...
    private void initFrame() {
        ...
        mainPane.add(dimensions, BorderLayout.CENTER);
        mainPane.add(tabButtons(), BorderLayout.EAST);
        tabbedPanetop.add("Sizes", mainPane);
    ...
    private emcJPanel tabButtons() {
        ...
        pnlButtons.add(new RemoveInActive(this.dataRelation, InventoryItemDimension2Setup.class.getName()), gbc);
        y++;
