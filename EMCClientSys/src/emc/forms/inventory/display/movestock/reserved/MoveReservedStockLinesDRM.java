 ...
 * @author wikus
 */
public class MoveReservedStockLinesDRM extends emcDataRelationManagerUpdate {

    private boolean busy = false;

    public MoveReservedStockLinesDRM(emcGenericDataSourceUpdate tableDataSource, EMCUserData userData) {
    ...
    public void setFieldValueAt(int rowIndex, String columnIndex, Object aValue) {
        if (!busy) {
            ...
            if (!(Boolean) super.getHeaderTable().getFieldValueAt(super.getHeaderTable().getLastRowAccessed(), "split")) {
                ...
                ((MoveReservedStockMasterDRM) super.getHeaderTable()).setFieldFromLines(super.getHeaderTable().getLastRowAccessed(), "quantity", value);
            } else {
                super.setFieldValueAt(rowIndex, columnIndex, aValue);
            }
            busy = false;
