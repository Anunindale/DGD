...
public class TRECCustomerChemicalsBean extends EMCEntityBean implements TRECCustomerChemicalsLocal {
    ...
    public void updateEncryptedFields(long chemRecId, EMCUserData userData) {
        ...
        if (chemical == null) {
            Logger.getLogger("emc").log(Level.SEVERE, "Failed to find the customer chemical.", userData);
            return;
        }
    ...
    public long createCustomerChemical(long trecLineRecId, String customer, String unNumber, String shippingName, EMCUserData userData) throws EMCEntityBeanException {
        ...
        if (chemical == null) {
            //Check if chemical exists whith difference Shipping Name
            query = new EMCQuery(enumQueryTypes.SELECT, TRECChemicals.class);
            query.addAnd("unNumber", unNumber);
            if (util.exists(query, userData)) {
                Logger.getLogger("emc").log(Level.SEVERE, "Failed to find the TREC Chemical. Please check that you have a valid shipping name.", userData);
            } else {
    ...
    public Object update(Object uobject, EMCUserData userData) throws EMCEntityBeanException {
        ...
        //Unapprove TREC Line
        List<TRECTrecCardsLines> trecLinesList = util.executeGeneralSelectQuery(query, userData);
        //this hange has been done to handle 2 cards linking to the same chemical eg when customer changes their details  a new card is generated linking to the same chemical
        if (trecLinesList.isEmpty()) {
            throw new EMCEntityBeanException("Failed to find the TREC Line.");
        } else {

            for (TRECTrecCardsLines trecLine : trecLinesList) {
