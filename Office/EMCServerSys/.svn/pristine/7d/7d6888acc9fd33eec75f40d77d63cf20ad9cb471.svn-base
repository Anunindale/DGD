...
//            populateDS(activePhraseString, phraseList, dataList, "SPECIAL INFORMATION FOR EMERGENCY SERVICES", line, chemical, userData);
    ...
    private void populateDS(String activePhraseString, List<TRECPhraseHelper> phrseList, List<TRECProducedCardTemplateDS> dataList, String heading, TRECTrecCardsLines line, TRECChemicalsSuper chemical, EMCUserData userData) {
        ...
        for (Object phraseHelper : phrseList) {
            ...
            ds.setForm(getAppearanceString(line.getForm(), null, null, userData));
            ds.setColour(getAppearanceString(null, line.getColour(), null, userData));
            ds.setOdour(getAppearanceString(null,null,line.getOdour(), userData));
            ...
            if (isBlank(che)) {
                ...
                if (prefShipName.isEmpty()) {
                    ds.setPreferredShippingNameNA("Not Applicable");
                } else {
                    int count = 0;
                    for (int j = 0; j < prefShipName.size(); j++) {
                        
                        if (prefShipName.get(j) != null && count==0) {
                            if (isBlank(ds.getPreferredShippingName0())) {
                                ds.setPreferredShippingName0((String) prefShipName.get(j));
                            }
                        }
                        if (prefShipName.get(j) != null && count==1) {
                             if (isBlank(ds.getPreferredShippingName1())) {
                                ds.setPreferredShippingName1((String) prefShipName.get(j));
                            }
                        }
                        if (prefShipName.get(j) != null && count==2) {
                             if (isBlank(ds.getPreferredShippingName2())) {
                                ds.setPreferredShippingName2((String) prefShipName.get(j));
                            }
                        }
                        if (prefShipName.get(j) != null && count==3) {
                             if (isBlank(ds.getPreferredShippingName3())) {
                                ds.setPreferredShippingName3((String) prefShipName.get(j));
                            }
                        }
                        if (prefShipName.get(j) != null && count==4) {
                             if (isBlank(ds.getPreferredShippingName4())) {
                                ds.setPreferredShippingName4((String) prefShipName.get(j));
                            }
                        }
                        if (prefShipName.get(j) != null && count==5) {
                             if (isBlank(ds.getPreferredShippingName5())) {
                                ds.setPreferredShippingName5((String) prefShipName.get(j));
                            }
                        }
                        count++;
                    }
