 ...
 */
package emc.forms.debtors.display.royaltygroups;
 ...
 * @author riaan
 */
public class RoyaltyGroupsForm extends BaseInternalFrame {

    private emcDataRelationManagerUpdate drm;

    /** Creates a new instance of RoyaltyGroupsForm */
    public RoyaltyGroupsForm(EMCUserData userData) {
        super("Royalty Groups", true, true, true, true, userData);
        this.setBounds(20, 20, 550, 290);

        drm = new emcDataRelationManagerUpdate(new emcGenericDataSourceUpdate(enumEMCModules.DEBTORS.getId(), new DebtorsRoyaltyGroups(), userData), userData);

        drm.setTheForm(this);
        this.setDataManager(drm);

        drm.setFormTextId1("royaltyGroupId");
