 ...
 * @author wikus
 */
public class InventoryMoveReservedStockForm extends BaseInternalFrame {

    private MoveReservedStockMasterDRM masterDRM;
    private MoveReservedStockLinesDRM linesDRM;
    private EMCUserData masterUD;
    private EMCUserData linesUD;
    private EMCLookup lkpLocation;
    private EMCLookup lkpWarehouse;
    private ReservedMovementLRM lrm;

    public InventoryMoveReservedStockForm(EMCUserData userData) {
        super("Move Reserved Stock", true, true, true, true, userData);
        ...
        try {
            ...
            masterDRM = new MoveReservedStockMasterDRM(new emcGenericDataSourceUpdate(
                    enumEMCModules.INVENTORY.getId(), new InventoryMoveStockMasterDS(), masterUD), param, masterUD);
            linesDRM = new MoveReservedStockLinesDRM(new emcGenericDataSourceUpdate(
    ...
    private emcJPanel controllPane() {
        ...
        lkpLocation.setValue("REC");
        lkpLocation.setEnabled(false);

            

        Component[][] comp = {{new emcJLabel()}, {new emcJLabel("Warehouse"), lkpWarehouse, new emcJLabel("Location"), lkpLocation}};
        ...
        emcJButton btnMove = new emcJButton("Move") {
            ...
            public void doActionPerformed(ActionEvent evt) {
                ...
                if (!Functions.checkBlank(warehouse) && !Functions.checkBlank(location)) {
                    EMCCommandClass cmd = new EMCCommandClass(EMCCommands.SERVER_GENERAL_COMMAND.getId(), enumEMCModules.INVENTORY.getId(), ServerInventoryMethods.MOVE_RESERVED_STOCK.toString());
        ...
        emcJButton btnFetch = new emcJButton("Get Data") {
            ...
            public void doActionPerformed(ActionEvent evt) {
                ...
                if (Functions.checkBlank(warehouse) || Functions.checkBlank(location)) {
                    ...
                    new ReservedDataFetchThread(progressBar, masterDRM, warehouse.toString(), location.toString(), "", "");
                }
            }
        };
...
//        keys.add("posted");
        ...
        table.setColumnEditable("reserved", false);
